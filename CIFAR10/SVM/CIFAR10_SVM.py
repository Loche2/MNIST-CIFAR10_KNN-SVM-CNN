...
def CIFAR10_DATASET_TRAIN(downloads, train_amount):
    # Load dataset
    ...
    train_data = np.array([np.transpose(np.array(img), (1, 2, 0)) for img, _ in training_data])
    train_label = np.array([label for label in training_data.targets])
...
# Load the test data
def CIFAR10_DATASET_TEST(downloads, test_amount):
    # Load dataset
    ...
    test_data = np.array([np.transpose(np.array(img), (1, 2, 0)) for img, _ in testing_data][:test_amount])
    test_label = np.array([label for label in testing_data.targets][:test_amount])
...
# Main function for CIFAR-10 dataset
if __name__ == '__main__':
    # Training Arguments Settings
    parser = argparse.ArgumentParser(description='Saak')
    parser.add_argument('--download_CIFAR10', default=True, metavar='DL',
                        help='Download CIFAR10 (default: True)')
    ...
    # Load Training Data & Testing Data for CIFAR-10
    train_data, train_label = CIFAR10_DATASET_TRAIN(args.download_CIFAR10, args.train_amount)
    test_data, test_label = CIFAR10_DATASET_TEST(args.download_CIFAR10, args.test_amount)

    training_features = train_data.reshape(len(train_data), -1)
